...
export default function ProductEdit() {
  ...
  const [form, setForm] = useState({
    ...
    title: "",
    slug: "",
    thumbnail: null,           // will hold a File object if new image chosen, else null
    price: "",
    subcategory_id: "",
    program_id: "",
    promote_text: [],
  ...
  // Fetch product data and dropdowns on mount
  useEffect(() => {
    fetchProduct();
    fetchSubcategories();
    fetchPrograms();
  ...
  const fetchProduct = async () => {
    ...
    setForm({
      ...
      title: data.title || "",
      slug: data.slug || "",
      thumbnail: null,                // keep null so we know no new file selected yet
  ...
  const handlePromoteTextChange = (index, value) => {
    const updated = [...form.promote_text];
    updated[index] = value;
    setForm((prev) => ({ ...prev, promote_text: updated }));
  };

  const handlePromoteTextRemove = (index) => {
    const updated = form.promote_text.filter((_, i) => i !== index);
    setForm((prev) => ({ ...prev, promote_text: updated }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    setUploading(true);
    try {
      let imageUrl = previewUrl;   // keep existing if no new file
      ...
      const payload = {
        ...
        promote_text: form.promote_text.filter(Boolean),
      ...
      <div className="w-full px-4">
        <div className="relative flex flex-col min-w-0 break-words w-full mb-6 shadow-lg rounded-lg bg-blueGray-100 border-0">
          <div className="rounded-t bg-white mb-0 px-6 py-6">
            <h6 className="text-blueGray-700 text-xl font-bold">Edit Product</h6>
          </div>
              ...
              <div className="flex flex-wrap mt-3">
                ...
                <div className="w-full lg:w-6/12 px-4">
                  ...
                  <div className="relative w-full mb-3">
                    ...
                    {form.promote_text.map((text, index) => (
