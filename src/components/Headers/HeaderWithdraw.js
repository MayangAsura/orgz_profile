...
export default function HeaderWithdraw() {
  ...
  const getBalance = async (period = null) => {
    if(period){
      let { data: orgz_orders, error } = await supabase
                                        ...
                                        .lt('created_at', "now() - interval '30 day'")
                                        .is('deleted_at', null)
                                        .single()

      if(orgz_orders){
        setBalance(total_debit - total_credit)
      }
  ...
  const getTotalCredit = async () => {
    ...
    if(orgz_cash_flows){
      console.log('orgz_orders', orgz_cash_flows)
      setTotalKredit(orgz_cash_flows.sum)
    }
